...
-- Insert default routing matrix
WITH request_type_data AS (
  SELECT id, name FROM public.request_types
),
routing_data AS (
  SELECT 
    ...
    role,
    permission_type
  FROM request_type_data rt
  CROSS JOIN (VALUES
    ('agent', CASE rt.name 
      WHEN 'Réservation/Commercial' THEN 'direct_treatment'
      ELSE 'entry_point' 
    END),
    ('technicien', CASE rt.name 
      WHEN 'Problème technique' THEN 'direct_treatment'
      ELSE 'no_access' 
    END),
    ('femme_de_menage', CASE rt.name 
      WHEN 'Ménage/Propreté' THEN 'direct_treatment'
      ELSE 'no_access' 
    END),
    ('comptabilite', CASE rt.name 
      WHEN 'Facturation' THEN 'direct_treatment'
      ELSE 'no_access' 
    END),
    ('superviseur', CASE rt.name 
      WHEN 'Réservation/Commercial' THEN 'read_only'
      WHEN 'Réclamation urgente' THEN 'direct_treatment'
      ELSE 'escalation' 
    END)
  ) AS roles(role, permission_type)
)
INSERT INTO public.routing_matrix (request_type_id, role, permission_type)
SELECT request_type_id, role, permission_type FROM routing_data;
