...
CREATE TABLE public.profiles (
  ...
  geographical_zones geographical_zone[] DEFAULT ARRAY['paris_centre'],
...
CREATE TABLE public.permissions (
  ...
  description TEXT,
  category TEXT NOT NULL, -- 'dashboard', 'chat', 'analytics', 'users', 'settings'
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
USING (
  EXISTS (
    ...
    AND profile_type IN ('administrateur')
  )
);
...
-- Create policies for permissions (read-only for non-admins)
...
-- Administrateur permissions (all permissions)
INSERT INTO public.profile_permissions (profile_type, permission_id)
SELECT 'administrateur', id FROM public.permissions;

-- Responsable logistique permissions
INSERT INTO public.profile_permissions (profile_type, permission_id)
...
-- Responsable qualité permissions  
INSERT INTO public.profile_permissions (profile_type, permission_id)
